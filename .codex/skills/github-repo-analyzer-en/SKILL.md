---
name: github-repo-analyzer-en
description: Analyze GitHub repositories and produce structured English reports covering project direction/positioning, capabilities, architecture, implementation approach and highlights. Use when the user asks to analyze or summarize one or many GitHub repos, build a repo survey/dossier (e.g. from a list of URLs), compare open-source projects, or understand what a repo does and how it is built, and the output report should be in English.
---

# GitHub Repo Analyzer (English)

Analyze GitHub repos (single or batch) and write per-repo Markdown reports in English.
...
3. **Analyze the repo**
   - Read `README.md` first, then inspect the top-level tree (`find repo -maxdepth 2`), manifests (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `requirements.txt`, etc.), `docs/` if present, and the entry point plus core modules.
   - Establish, with file-level evidence:
     - Direction: domain, positioning, target audience
     - Capabilities: what it can do (capability list)
     - Architecture: module layout, layers, data flow / execution flow
     - Implementation approach and core logic: key design decisions, where core logic lives
     - Highlights: technical / engineering / usability
...
4. **Write the report**
   ...
   - Output: `reports/<owner>__<repo>.md` (English content, code identifiers kept as-is).
   - Order matters: the non-technical section and category come right after the header block, before direction & positioning, so non-technical readers get the gist early.
